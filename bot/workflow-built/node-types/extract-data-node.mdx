...
- Accumulating data from multiple pages when combined with [Pagination](/bot/workflow-built/node-types/pagination-node) inside a [Loop](/bot/workflow-built/node-types/loop-node).
...
- Returns structured data in the default output format configured in the **Start** node.
...
5. For cross-page extraction, place Extract Data inside a [Loop](/bot/workflow-built/node-types/loop-node) or combine with [Pagination](/bot/workflow-built/node-types/pagination-node).
...
- Each Extract Data node executes extraction only once. For multi-page extraction, combine with [Loop](/bot/workflow-built/node-types/loop-node) + [Pagination](/bot/workflow-built/node-types/pagination-node).
- Consider adding a [**Wait**](/bot/workflow-built/node-types/wait-node) node before extraction to ensure dynamic content has loaded.
