...
[![Watch How to Scrape Deep Nested Data with Loop List](https://i.ytimg.com/vi/tarseJbeSII/hqdefault.jpg)](https://www.youtube.com/watch?v=tarseJbeSII)

[Watch **How to Scrape Deep Nested Data with Loop List** on YouTube](https://www.youtube.com/watch?v=tarseJbeSII).
...
3. Add child nodes inside the Loop List:
   - [**Extract Data Item**](/bot/workflow-built/node-types/extract-data-item-node)**:** Extract fields from the current list item.
   - [**Click Element Item**](/bot/workflow-built/node-types/click-element-item-node)**:** Click a button or link within the current list item.
...
- Loop List requires at least one child node ([**Extract Data Item**](/bot/workflow-built/node-types/extract-data-item-node) or [**Click Element Item**](/bot/workflow-built/node-types/click-element-item-node)).
- Setting **Max Focused Loop Items** too high may result in excessive execution time; match it to actual requirements.
- For infinite scroll lists, combine with [**Scroll**](/bot/workflow-built/node-types/scroll-page-node) nodes and the "Load More" option.
