...
Extract Data Item is a specialized child node designed for use inside a [Loop List](/workflow/learn/node-types/loop-list-node). It extracts specified fields from the currently focused single list item and compiles these fields into structured data.
...
1. Add an **Extract Data Item** node inside a [**Loop List**](/workflow/learn/node-types/loop-list-node).
...
- If the page structure is complex, consider splitting into multiple [Loop List](/workflow/learn/node-types/loop-list-node) + Extract Data Item sets to extract by region.