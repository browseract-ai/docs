...
3. Add child nodes inside the Loop List:
   - [**Extract Data Item**](/workflow/learn/node-types/extract-data-item-node)**:** Extract fields from the current list item.
   - [**Click Element Item**](/workflow/learn/node-types/click-element-item-node)**:** Click a button or link within the current list item.
...
- Loop List requires at least one child node ([**Extract Data Item**](/workflow/learn/node-types/extract-data-item-node) or [**Click Element Item**](/workflow/learn/node-types/click-element-item-node)).
- Setting **Max Focused Loop Items** too high may result in excessive execution time; match it to actual requirements.
- For infinite scroll lists, combine with [**Scroll**](/workflow/learn/node-types/scroll-page-node) nodes and the "Load More" option.
