...
- Scroll Page scrolls only once per execution. Combine with a [**Loop**](/workflow/learn/node-types/loop-node) node for continuous scrolling.
- For pages using "Scroll down to load more," you can loop: \*\*Scroll Page → \*\*[**Extract Data**](/workflow/learn/node-types/extract-data-node).
- If the goal is to scroll to a specific area (e.g., comments section), consider using the [**Scroll to Element**](/workflow/learn/node-types/scroll-to-element-node) node instead.