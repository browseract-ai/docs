...
- Accumulating data from multiple pages when combined with [Pagination](/workflow/learn/node-types/pagination-node) inside a [Loop](/workflow/learn/node-types/loop-node).
...
5. For cross-page extraction, place Extract Data inside a [Loop](/workflow/learn/node-types/loop-node) or combine with [Pagination](/workflow/learn/node-types/pagination-node).
...
- Each Extract Data node executes extraction only once. For multi-page extraction, combine with [Loop](/workflow/learn/node-types/loop-node) + [Pagination](/workflow/learn/node-types/pagination-node).
- Consider adding a [**Wait**](/workflow/learn/node-types/wait-node) node before extraction to ensure dynamic content has loaded.
