...
1. Click **\+** on the canvas (or right-click) and choose [**Visit Page**](/workflow/learn/node-types/visit-node).
...
1. Add an [**Input Text**](/workflow/learn/node-types/input-text-node) node and connect it after **Visit Page**.
...
Use one or more [**Click Element**](/workflow/learn/node-types/click-element-node) nodes to perform these three clicks in order:
...
1. Add an [**Extract Data**](/workflow/learn/node-types/extract-data-node) node after the last Click Element node.
...
1. Add a **Finish: Output Data** node and connect it after [**Extract Data.**](/workflow/learn/node-types/extract-data-node)
...
- **CSV is empty or has very few rows:**
  - Check the sample in [**Extract Data**](/workflow/learn/node-types/extract-data-node): it should be a full list item, not a tiny sub-element.
  - **Wait for load:** Add a short [wait](/workflow/learn/node-types/wait-node) before Extract Data if the list loads slowly.
- **Click Element does nothing:**
  - Re-select the actual clickable element (button/text) in the [Click Element node](/workflow/learn/node-types/click-element-node).
...
- **Run error and not sure why:**
  ...
  - Ensure no node has missing config ([Click Element](/workflow/learn/node-types/click-element-node), etc.).
