...
- **CSV is empty or has very few rows:**
  ...
  - **Wait for load:** Add a short wait (sleep) before Extract Data if the list loads slowly.
...
- **Run error and not sure why:**
  ...
  - Ensure no node has missing config (selectors, fields, etc.).

> **Support:** If it still fails, contact support via **“Contact Us” / “Help Center”** with:
>
...
2. Clone the pattern for a new scenario

   Use the same structure (Visit → Search → Filter → Extract → Finish) for:
   - Hotel listings
   - Job boards
   - Product lists on e-commerce sites
